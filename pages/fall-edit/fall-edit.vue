			...
			<view class="form-item">
				...
				<view class="form-cnt form-mid-space2">
					<picker mode="selector" :range="patientListRange"  @change="setPatient">
						<input class="form-mid-space" disabled="true" type="text" v-model="data.patient_id + ' ' + data.patient_name" name="usedate"/>
					</picker>
				</view>
				<view class="form-cnt form-mid-space">
					<input class="form-mid-space" type="text" name="usedate" @input="inputChange" placeholder-style="font-size:28rpx;" placeholder="搜索患者"/>
				</view>
			</view>
			<view class="form-item">
				<view class="form-prefix-space">
...
<script>
	...
	export default {
		...
		methods: {
			...
			//获取跌倒风险评估数据
			getFallDetail(){
				this.data = Store.getStorageSync("fallItem");
				console.log("fallItem",this.data)
				//处理患者id
				for (let k = 0; k < this.patientListInfo.length; k++) {
					if(this.data.patient_name === this.patientListInfo[k].name){
						this.data.patient_id = this.patientListInfo[k].patient_id;
						break;
					}
				}
				//处理年龄评分
				for (let l = 0; l < this.ageList.length; l++) {
					if(this.data.age_score === this.ageList[l].value){
						this.ageList[l].check = true;
					}
				}
				//处理跌倒史评分
				for (let m = 0; m < this.fallList.length; m++) {
					if(this.data.fall_history_score === this.fallList[m].value){
						this.fallList[m].check = true;
					}
				}
				//处理意识状况评分
				for (let n = 0; n < this.consciousList.length; n++) {
					if(this.data.conscious_score === this.consciousList[n].value){
						this.consciousList[n].check = true;
					}
				}
				//处理活动情况评分
				for (let o = 0; o < this.moveList.length; o++) {
					if(this.data.mobility_score === this.moveList[o].value){
						this.moveList[o].check = true;
					}
				}
				//处理视力评分
				for (let p = 0; p < this.sightList.length; p++) {
					if(this.data.eyesight_score === this.sightList[p].value){
						this.sightList[p].check = true;
					}
				}
				//处理疾病评分
				for (let q = 0; q < this.diseaseList.length; q++) {
					if(this.data.disease_score === this.diseaseList[q].value){
						this.diseaseList[q].check = true;
					}
				}
				//处理药物评分
				for (let r = 0; r < this.drugList.length; r++) {
					if(this.data.drug_score === this.drugList[r].value){
						this.drugList[r].check = true;
					}
				}
			},
			//修改跌倒风险评估数据
			setFallDetail(){
				this.$myRequest({
					...
					success: (res) => {
						if(res.data.code == 200){
							console.log("修改跌倒风险评估",res.data.data)
							uni.showToast({
								title: '跌倒风险评估修改成功！',
								icon: 'none',
								mask: true
